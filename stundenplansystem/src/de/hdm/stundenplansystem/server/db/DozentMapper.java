...
/**
 * Mapper-Klasse, die <code>Dozent</code>-Objekte auf eine relationale Datenbank
 ...
 * 
 * { @link LehrveranstaltungMapper, RaumMapper, SemesterverbandMapper,
...
public class DozentMapper {
	 ...
	 * einzige Instanz von <code>DozentMapper</code> existiert.
	 * <p>
	 * 
	...
	/**
	 * Suchen eines Dozenten mit vorgegebener id. Da diese eindeutig ist, wird
	 * genau ein Objekt zurückgegeben.
	 * 
	 * @param id
	 *            Primarschlüsselattribut (->DB)
	...
	public Dozent findByKey(int id) {
		...
		try {
			...
			ResultSet rs = stmt
					.executeQuery("SELECT id, vorname, nachname FROM dozent "
							+ "WHERE id=" + id);
	...
	public Vector<Dozent> findAll() {
		...
		try {
			...
			ResultSet rs = stmt
					.executeQuery("SELECT id, nachname, vorname FROM dozent "
							+ " ORDER BY id");

			// Für jeden Eintrag im Suchergebnis wird nun ein Dozent-Objekt
			// erstellt.
			while (rs.next()) {
				Dozent d = new Dozent();
				d.setId(rs.getInt("id"));
	...
	public Dozent insert(Dozent d) {
		...
		try {
			...
			if (rs.next()) {
				...
				stmt.executeUpdate("INSERT INTO dozent (id, nachname, vorname) "
						+ "VALUES ("
						+ d.getId()
						+ ",'"
						+ d.getNachname()
	...
	public Dozent update(Dozent d) {
		...
		try {
			...
			stmt.executeUpdate("UPDATE dozent SET " + "vorname= '"
					+ d.getVorname() + "', " + "nachname= " + "'"
					+ d.getNachname() + "' " + "WHERE id="
					+ d.getId());
	...
	public void delete(Dozent d) {
		...
		try {
			...
			stmt.executeUpdate("DELETE FROM dozent " + "WHERE id="
					+ d.getId());
