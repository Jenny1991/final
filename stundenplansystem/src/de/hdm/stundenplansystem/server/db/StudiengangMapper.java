...
public class StudiengangMapper {
	...
	/**
	 * Die Klasse StudiengangMapper wird nur einmal instantiiert. Man spricht
	 ...
	 * einzige Instanz von <code>StudiengangMapper</code> existiert.
	 * <p>
	 * 
	...
	public Studiengang findByKey(int id) {
		...
		try {
			...
			ResultSet rs = stmt
					.executeQuery("SELECT id, bezeichnung FROM studiengang "
							+ "WHERE id=" + id);
	...
	public Studiengang findBySemesterverbandId(int semesterverbandid) {
		...
		try {
			...
			// Statement ausfüllen und als Query an die DB schicken
			ResultSet rs = stmt
					.executeQuery("SELECT studiengang.id, studiengang.bezeichnung FROM studiengang "
			...
			if (rs.next()) {
				...
				s.setBezeichnung(rs
						.getString("studiengang.bezeichnung"));
	...
	public Vector<Studiengang> findAll() {
		...
		try {
			...
			ResultSet rs = stmt
					.executeQuery("SELECT id, bezeichnung "
							+ "FROM studiengang " + " ORDER BY id");

			// Für jeden Eintrag im Suchergebnis wird nun ein Studiengang-Objekt
			// erstellt.
			while (rs.next()) {
				Studiengang s = new Studiengang();
				s.setId(rs.getInt("id"));
	...
	/**
	 * Einfügen eines <code>Studiengang</code>-Objekts in die Datenbank. Dabei
	...
	public Studiengang insert(Studiengang s) {
		...
		try {
			...
			if (rs.next()) {
				...
				stmt.executeUpdate("INSERT INTO studiengang (id, bezeichnung)"
						+ "VALUES ("
						+ s.getId()
						+ ",'"
						+ s.getBezeichnung() + "')");
		 ...
		 * Objekte übergeben werden, wäre die Anpassung des Studiengang-Objekts
	...
	/**
	 * Löschen der Daten eines <code>Studiengang</code>-Objekts aus der
	 * Datenbank.
	 * 
	 * @param s
	...
	public void delete(Studiengang s) {
		...
		try {
			...
			stmt.executeUpdate("DELETE FROM studiengang "
					+ "WHERE id=" + s.getId());
