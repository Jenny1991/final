...
public class StundenplanMapper {
	...
	/**
	 * Die Klasse StundenplanMapper wird nur einmal instantiiert. Man spricht
	 ...
	 * einzige Instanz von <code>StundenplanMapper</code> existiert.
	 * <p>
	 * 
	 ...
	 * @return Stundenplan-Objekt, das dem übergebenen Schlüssel entspricht,
	...
	public Stundenplan findByKey(int id) {
		...
		try {
			...
			ResultSet rs = stmt
					.executeQuery("SELECT id, studienhalbjahr, semesterverbandid FROM stundenplan "
							+ "WHERE id=" + id);
	...
	public Vector<Stundenplan> findAll() {
		...
		try {
			...
			ResultSet rs = stmt
					.executeQuery("SELECT id, studienhalbjahr, semesterverbandid "
							+ "FROM stundenplan " + " ORDER BY id");
	...
	/**
	 * Einfügen eines <code>Stundenplan</code>-Objekts in die Datenbank. Dabei
	...
	public Stundenplan insert(Stundenplan s) {
		...
		try {
			...
			if (rs.next()) {
				...
				stmt.executeUpdate("INSERT INTO stundenplan (id, studienhalbjahr, semesterverbandid) "
						+ "VALUES ("
						+ s.getId()
						+ ",'"
						+ s.getStudienhalbjahr()
						+ "',"
						+ s.getSemesterverbandId() + ")");
			}
		} catch (SQLException e2) {
			e2.printStackTrace();
		}
		 ...
		 * Objekte übergeben werden, wäre die Anpassung des Stundenplan-Objekts
	...
	/**
	 * Löschen der Daten eines <code>Stundenplan</code>-Objekts aus der
	 * Datenbank.
	 * 
	 * @param s
	...
	public void delete(Stundenplan s) {
		...
		try {
			...
			stmt.executeUpdate("DELETE FROM stundenplan "
					+ "WHERE id=" + s.getId());
